...
export interface MapPoint {
  ...
  position: LatLon
  tooltip?: ReactNode
  /** Pin color. Defaults to the design system's brand accent, or its secondary tone when `emphasized`. */
  color?: string
...
export function MapMarker({ point }: MapMarkerProps) {
  ...
  const icon = useMemo(() => {
    ...
    return createMapIcon(point.color ?? (point.emphasized ? theme.info : theme.primary), size)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [point.iconUrl, point.color, point.emphasized, size[0], size[1], theme.primary, theme.info])
